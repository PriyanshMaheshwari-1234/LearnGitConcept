...
import com.ctl.it.qa.eshop.steps.ChangeSteps;
import com.ctl.it.qa.eshop.steps.OrderSteps;

import cucumber.api.java.en.Then;
import cucumber.api.java.en.And; //Sachin added
...
//ac41685
	...
	// Swati added to remove HSI from HSI+POTS product without removal reason

	@Then("^I Remove the existing product HSI without removal reason$")
	public void i_Remove_the_existing_product_HSI_without_removal_reason() {
		changesteps.i_removed_existing_product_HSI_without_removal_reason();
	}
	
	
	//created by sachin	
	@And("^I select No PaperLess billing option$")
	public void i_select_no_paperless_billing_option() throws Throwable {
		
		changesteps.selectBillingOption();
	}

	//created by sachin
	@Then("^I should be in Product offer page while Move$")
	public void i_should_be_in_Product_offer_page_while_Move() {
		try {
			changesteps.verfyPickyourOffer();  
		} catch (Exception ex) {
			Assert.fail();
		}
	}
