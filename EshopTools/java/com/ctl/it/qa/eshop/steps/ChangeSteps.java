...
public class ChangeSteps extends Steps {
	...
	public void i_removed_existing_product_CVOIP() {
		waitABit(20000);//Prathibha added delay
		WaitForPageToLoad(60);
		JavascriptExecutor jse1 = (JavascriptExecutor) getDriver();
		jse1.executeScript("arguments[0].click();",changepage.icon_addphone);
		JavascriptExecutor jse2 = (JavascriptExecutor) getDriver();
	...
	//AB18364
			...
			//swati added step for removing HSI without removal reason
			
			@Step
			public void i_removed_existing_product_HSI_without_removal_reason() {
			...
			{
				slf4jLogger.info(" Select Radio button No mal bills selected");
				WebElement RadioButton = getDriver()
						                 .findElement(By.id("no-paperless"));
			        RadioButton.click();
			        slf4jLogger.info(" Radio button No mal bills selected");
			}
			
			//Created by Sachin
			@Step
			public void verfyPickyourOffer() throws InterruptedException {
				waitABit(30000);
			//	orderPage.shouldExist(orderPage, 60);
				String PickyourOfferText = "Pick your Offer ";
				Assert.assertTrue(changepage.PickyourOffer.getText().trim().contains(PickyourOfferText));
				slf4jLogger.info("Verified Pick your Offer page");
				slf4jLogger.info("Now in Eshop Environment  " + changepage.getCurrentEnvironment());
