...
public class VacationSuspendSteps extends Steps {
	...
	//Sachin changes for suspend
	
	public void i_See_Suspend_Restore_Page()
	{
		waitABit(10000);
		String SuspendRestoreTittleText = "Suspend & Restore";
		Assert.assertTrue(vacationSuspendPage.SuspendRestorePageTittle.getText().trim().contains(SuspendRestoreTittleText));
		slf4jLogger.info("I Verified Suspend or Restore page");
	
		waitABit(15000);
	}
	
	public void i_Click_On_SuspendAll()
	{
		waitABit(5000);
		String ExistingBanStatustext = "Active";
		Assert.assertTrue(vacationSuspendPage.BeforeOpBanStatus.getText().trim().contains(ExistingBanStatustext));
		slf4jLogger.info("Verifed BAN is in Active state");
		waitABit(5000);	
		vacationSuspendPage.ClickOnSuspendAll.click();
		slf4jLogger.info("I Clicked on SuspendALL ");
        waitABit(5000);
		String AfterClickOnSuspendALLBanStatustext = "Ready to Suspend";
		Assert.assertTrue(vacationSuspendPage.AfterOpBanStatus.getText().trim().contains(AfterClickOnSuspendALLBanStatustext));
		slf4jLogger.info("Verifed BAN is in Ready to Suspend state");
		waitABit(5000);
	}

	public void i_Click_On_Submit_On_Suspend_Restore_Page() 
	{
		
		waitABit(5000);
		vacationSuspendPage.SubmitButton.click();
	    waitABit(5000);
	   vacationSuspendPage.ConfirmSubmitButton.click();
		slf4jLogger.info("Non Pay Suspend Order/restore Order submited successfully ");
		waitABit(15000);
	...
	//Priyansh added
	@Step
	 public void clickContinueOnPage() {
		 waitABit(4000);
		 vacationSuspendPage.prodAndServ.isDisplayed();
		 vacationSuspendPage.prodAndServContinue.click();
	}
